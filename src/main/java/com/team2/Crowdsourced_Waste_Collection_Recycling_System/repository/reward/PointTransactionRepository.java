...
public interface PointTransactionRepository extends JpaRepository<PointTransaction, Integer> {
    ...
    @EntityGraph(attributePaths = { "citizen", "citizen.user", "collectionRequest", "collectionRequest.enterprise",
            "collectionRequest.report", "createdBy" })
    ...
    /**
     * Lấy bảng xếp hạng Citizen dựa trên tổng điểm tích lũy (chỉ tính điểm kiếm
     * được 'EARN').
    ...
    void deleteByCreatedBy_Id(Integer userId);
}
